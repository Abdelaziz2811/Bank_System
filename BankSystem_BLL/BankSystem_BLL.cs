...
namespace BankSystem_BLL
{
    ...
    public class clsClient_BLL
    {
        ...
        public static bool Transfer(clsClient_BLL Receiver, clsClient_BLL Deliver, decimal Amount)
        {
            if (Deliver.Balance < Amount)
                return false;
            else
                Deliver.Balance -= Amount;

            Receiver.Balance += Amount;

            return Deliver.Save() && Receiver.Save();
        }
    }

    public class clsTransferLogs_BLL
    {
        public int TransferLogID { set; get; }
        public int DeliverAccount { set; get; }
        public int ReceiverAccount { set; get; }
        public decimal Amount { set; get; }
        public decimal DeliverAccBalance { set; get; }
        public decimal ReceiverAccBalance { set; get; }
        public DateTime TransferDate { set; get; }
        public int ManagerID { set; get; }

        public clsTransferLogs_BLL(int DeliverAccount, int ReceiverAccount, decimal Amount, decimal DeliverAccBalance,
            decimal ReceiverAccBalance, DateTime TransferDate, int ManagerID)

        {
            this.TransferLogID = 0;
            this.DeliverAccount = DeliverAccount;
            this.ReceiverAccount = ReceiverAccount;
            this.Amount = Amount;
            this.DeliverAccBalance = DeliverAccBalance;
            this.ReceiverAccBalance = ReceiverAccBalance;
            this.TransferDate = TransferDate;
            this.ManagerID = ManagerID;
        }
        
        public bool SaveTransferLog()
        {
            this.TransferLogID = clsTransferLogs_DAL.SaveTransferLog(this.TransferLogID, this.DeliverAccount, this.ReceiverAccount, this.Amount, this.DeliverAccBalance,
                this.ReceiverAccBalance, this.TransferDate, this.ManagerID);

            return this.TransferLogID != 0;
        }

        public static DataTable GetTransferLogs()
        {
            return clsTransferLogs_DAL.GetTransferLogs();
        }
    }

    public class clsLoginRegister_BLL
    {
        public int LoginRegisterID { set; get; }
        public int UserID { set; get; }
        public DateTime Date { set; get; }

        public clsLoginRegister_BLL(int UserID, DateTime Date)
        {
            this.LoginRegisterID = 0;
            this.UserID = UserID;
            this.Date = Date;
        }

        public bool SaveLoginRegister()
        {
            this.LoginRegisterID = clsLoginsRegister_DAL.SaveLoginRegister(this.UserID, this.Date);
            return this.LoginRegisterID != 0;
        }

        public static DataTable GetLoginsRegister()
        {
            return clsLoginsRegister_DAL.GetLoginsRegister();
        }
    }
}
