...
namespace BankSystem_DAL
{
    ...
    public class clsTransferLogs_DAL
    {
        public static DataTable GetTransferLogs()
        {
            ...
            return DTTransferLogs;
        }

        public static int SaveTransferLog(int TransferLogID, int DeliverAccount, int ReceiverAccount, decimal Amount, decimal DeliverAccBalance,
            decimal ReceiverAccBalance, DateTime TransferDate, int ManagerID)
        {
            SqlConnection connection = new SqlConnection(DALSettings.ConnectionString);

            string query = @"INSERT INTO TransferLogs VALUES 
                             (@DeliverAccount, @ReceiverAccount, @Amount,
                             @DeliverAccBalance, @ReceiverAccBalance,
                             @TransferDate, @ManagerID);  
                             SELECT SCOPE_IDENTITY();";

            SqlCommand command = new SqlCommand(query, connection);

            command.Parameters.AddWithValue("@DeliverAccount", DeliverAccount);
            command.Parameters.AddWithValue("@ReceiverAccount", ReceiverAccount);
            command.Parameters.AddWithValue("@Amount", Amount);
            command.Parameters.AddWithValue("@DeliverAccBalance", DeliverAccBalance);
            command.Parameters.AddWithValue("@ReceiverAccBalance", ReceiverAccBalance);
    ...
    public class clsLoginsRegister_DAL
    {
        public static int SaveLoginRegister(int UserID, DateTime Date)
        {
            SqlConnection connection = new SqlConnection(DALSettings.ConnectionString);

            string query = @"INSERT INTO LoginsRegister VALUES 
                             (@UserID, @Date);  
                             SELECT SCOPE_IDENTITY();";
        ...
        public static DataTable GetLoginsRegister()
        {
            ...
            string query = @"SELECT LoginsRegister.LoginRegisterID, LoginsRegister.Date,
                             LoginsRegister.UserID, Users.Username, Users.Name,
                             Users.Email, Users.Phone, Users.Address 
                             FROM LoginsRegister
                             inner join Users on LoginsRegister.UserID = Users.UserID";

            SqlCommand command = new SqlCommand(query, connection);

            DataTable DTLoginsRegister = new DataTable();
