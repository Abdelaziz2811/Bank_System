...
namespace BankSystem
{
    public partial class BankSystem : Form
    {
        ...
        enum enFeatures { ClientList = 1, FindClient = 2, AddClient = 4, DeleteClient = 8, UpdateClient = 16 }
        ...
        private void BankSystem_Load(object sender, EventArgs e)
        {
            ...
            AddCurrentUserInfoToLoginRegister();

            SetFeaturesAccess_ClientSection();
        }

        void SetFeaturesAccess_ClientSection()
        {
            if ((CurrentUser.Permissions & (int)enFeatures.ClientList) == (int)enFeatures.ClientList)
                GetClientsInfo();

            if ((CurrentUser.Permissions & (int)enFeatures.FindClient) != (int)enFeatures.FindClient)
            {
                BTNCSearch.Enabled = false;
                TXTBS_ClientID.Enabled = false;
            }

            if ((CurrentUser.Permissions & (int)enFeatures.AddClient) != (int)enFeatures.AddClient)
            {
                BTNAddClient.Enabled = false;
            }

            if ((CurrentUser.Permissions & (int)enFeatures.UpdateClient) != (int)enFeatures.UpdateClient)
            {
                BTNC_Update.Enabled = false;
            }
        }
        ...
        void SetDefaultTabs()
        {
            TCMainSections.SelectedIndex = 4;
        ...
        {
            int FeatureToBeAccess = Convert.ToInt32(TCClients.SelectedTab.Tag);
            if ((CurrentUser.Permissions & FeatureToBeAccess) != FeatureToBeAccess)
                return;
        ...
        clsClient_BLL D_Client;

        private void BTND_FindClient_Click(object sender, EventArgs e)
        {
            if (!string.IsNullOrWhiteSpace(TXTBD_ClientID.Text))
            {
                D_Client = clsClient_BLL.Find(Convert.ToInt32(TXTBD_ClientID.Text));

                if (D_Client != null)
                {
                    GetClientInfoToView_Deposit(D_Client);
                    TXTB_DepositeAmount.Enabled = true;
        ...
        {
            if (!string.IsNullOrWhiteSpace(TXTB_DepositeAmount.Text))
            {
                if (MessageBox.Show($"You're about to deposit '{TXTB_DepositeAmount.Text}$' to the Client with account number '{D_Client.AccountNumber}'", "Deposit", MessageBoxButtons.OKCancel, MessageBoxIcon.Information) == DialogResult.OK)
                {
                    if (D_Client.Deposit(Convert.ToDecimal(TXTB_DepositeAmount.Text)))
                    {
                        MessageBox.Show($"Deposit operation succeeded Deposit amount '{TXTB_DepositeAmount.Text}%'\n to the Client with account number '{D_Client.AccountNumber}'", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        LBD_ClientBalance.Text = D_Client.Balance.ToString();
                        GetClientsInfo();
                    }
                }
                else
                    MessageBox.Show($"Deposit amount '{TXTB_DepositeAmount.Text}$' to the Client with account number '{D_Client.AccountNumber}' has been canceled", "Operation Canceled", MessageBoxButtons.OK, MessageBoxIcon.Information);

            }
            else
                MessageBox.Show($"Please enter deposit amount to be perform\nfor the the Client with account number '{D_Client.AccountNumber}'", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }

        clsClient_BLL W_Client;

        private void BTNW_FindClient_Click(object sender, EventArgs e)
        {
            if (!string.IsNullOrWhiteSpace(TXTBW_ClientID.Text))
            {
                W_Client = clsClient_BLL.Find(Convert.ToInt32(TXTBW_ClientID.Text));

                if (W_Client != null)
                {
                    GetClientInfoToView_Withdraw(W_Client);
                    TXTB_WithdrawAmount.Enabled = true;
                    BTNWithdraw.Enabled = true;
                }
                else MessageBox.Show($"Client with ID {TXTBW_ClientID.Text} not found", "Error", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
            else MessageBox.Show($"Please Enter Client ID", "Error", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
        }

        void GetClientInfoToView_Withdraw(clsClient_BLL Client)
        {
            LBW_ClientID.Text = Client.ClientID.ToString();
            LBW_ClientName.Text = Client.ClientName;
            LBW_AccountNumber.Text = Client.AccountNumber.ToString();
            LBW_ManagerID.Text = Client.CManagerID.ToString();
            LBW_ClientPhone.Text = Client.ClientPhone;
            LBW_ClientEmail.Text = Client.ClientEmail;
            LBW_ClientBalance.Text = Client.Balance.ToString(); ;
            LBW_ClientAddress.Text = Client.ClientAddress;

            PBW_ClientImage.Load(Client.ClientImagePath);
        }

        private void TXTB_WithdrawAmount_KeyPress(object sender, KeyPressEventArgs e)
        {
            // Allow control keys like backspace
            if (!char.IsControl(e.KeyChar))
            {
        ...
        {
            if (!string.IsNullOrWhiteSpace(TXTB_WithdrawAmount.Text))
            {
                if (Convert.ToDecimal(TXTB_WithdrawAmount.Text) > W_Client.Balance)
                {
                    MessageBox.Show($"Amount exceeds the balance !! you can't withdraw the current amount\nbecause : {TXTB_WithdrawAmount.Text}$ > {W_Client.Balance}$", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
 
                if (MessageBox.Show($"You're about to withdraw '{TXTB_WithdrawAmount.Text}$'\nfrom the Client with account number '{W_Client.AccountNumber}'", "Withdraw", MessageBoxButtons.OKCancel, MessageBoxIcon.Information) == DialogResult.OK)
                {                    
                    if (W_Client.Deposit(Convert.ToDecimal(TXTB_WithdrawAmount.Text) * -1))
                    {
                        MessageBox.Show($"Withdraw operation succeeded Withdraw amount '{TXTB_WithdrawAmount.Text}$'\nfrom the Client with account number '{W_Client.AccountNumber}'", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        LBW_ClientBalance.Text = W_Client.Balance.ToString();
                        GetClientsInfo();
                    }
                    else
                        MessageBox.Show($"Withdraw operation failled", "Failled", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                else
                    MessageBox.Show($"Withdraw amount '{TXTB_WithdrawAmount.Text}$' from the Client with\naccount number '{W_Client.AccountNumber}' has been canceled", "Operation Canceled", MessageBoxButtons.OK, MessageBoxIcon.Information);

            }
            else
                MessageBox.Show($"Please enter withdraw amount to be perform\nfor the client with account number '{W_Client.AccountNumber}'", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }

        clsClient_BLL ReceiverClient;

        private void BTNTR_FindClient_Click(object sender, EventArgs e)
        {
            if (!string.IsNullOrWhiteSpace(TXTBTR_ClienID.Text))
            {
                ReceiverClient = clsClient_BLL.Find(Convert.ToInt32(TXTBTR_ClienID.Text));

                if (ReceiverClient != null)
                {
                    GetReceiverClientInfo(ReceiverClient);
                    
                    if (LBTR_ClientID.Text != "----------" && LBTD_ClientID.Text != "----------")
                    {
                        BTNTransfer.Enabled = true;
                        TXTBTransferAmount.Enabled = true;
                    }
                }
                else MessageBox.Show($"Client with ID {TXTBTR_ClienID.Text} not found", "Error", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
            else MessageBox.Show($"Please Enter Client ID", "Error", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
        }

        void GetReceiverClientInfo(clsClient_BLL ReceiverClient)
        {
            LBTR_ClientID.Text = ReceiverClient.ClientID.ToString();
            LBTR_Name.Text = ReceiverClient.ClientName;
            LBTR_Balance.Text = ReceiverClient.Balance.ToString();
            LBTR_AccountNumber.Text = ReceiverClient.AccountNumber.ToString();
            PBTR_ClientImage.Load(ReceiverClient.ClientImagePath);
        }

        clsClient_BLL DeliverClient;

        private void BTNTD_FindClient_Click(object sender, EventArgs e)
        {
            if (!string.IsNullOrWhiteSpace(TXTBTD_ClienID.Text))
            {
                DeliverClient = clsClient_BLL.Find(Convert.ToInt32(TXTBTD_ClienID.Text));

                if (DeliverClient != null)
                {
                    GetDeliverClientInfo(DeliverClient);
                ...
                else MessageBox.Show($"Client with ID {TXTBTD_ClienID.Text} not found", "Error", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
            else MessageBox.Show($"Please Enter Client ID", "Error", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
        }

        void GetDeliverClientInfo(clsClient_BLL DeliverClient)
        {
            LBTD_ClientID.Text = DeliverClient.ClientID.ToString();
            LBTD_Name.Text = DeliverClient.ClientName;
            LBTD_Balance.Text = DeliverClient.Balance.ToString();
            LBTD_AccountNumber.Text = DeliverClient.AccountNumber.ToString();
            PBTD_ClientImage.Load(DeliverClient.ClientImagePath);
        }

        private void BTNTransfer_Click(object sender, EventArgs e)
        {
            if (!string.IsNullOrWhiteSpace(TXTBTransferAmount.Text))
            {
                if (Convert.ToDecimal(TXTBTransferAmount.Text) > DeliverClient.Balance)
                {
                    MessageBox.Show($"Amount exceeds the balance !! you can't perform transfer\nbecause : Transfer amount : {TXTBTransferAmount.Text}$ > the Deliver Client Balance {DeliverClient.Balance}$", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }

                if (LBTD_ClientID.Text == LBTR_ClientID.Text)
                {
                    MessageBox.Show($"You can't perform transfer to the same account", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }

                if (MessageBox.Show($"You're about to transfer '{TXTBTransferAmount.Text}$' from the Client with account number '{DeliverClient.AccountNumber}' to the Client with account number '{ReceiverClient.AccountNumber}'", "Transfer", MessageBoxButtons.OKCancel, MessageBoxIcon.Information) == DialogResult.OK)
                {
                    decimal Amount = Convert.ToDecimal(TXTBTransferAmount.Text);
                    if (clsClient_BLL.Transfer(ReceiverClient, DeliverClient, Amount))
                    {
                        MessageBox.Show($"Transfer operation succeeded", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        LBTR_Balance.Text = ReceiverClient.Balance.ToString();
                        LBTD_Balance.Text = DeliverClient.Balance.ToString();
                        GetClientsInfo();

                        clsTransferLogs_BLL TransferLog = new clsTransferLogs_BLL( DeliverClient.AccountNumber,
                            ReceiverClient.AccountNumber, Amount, DeliverClient.Balance,
                            ReceiverClient.Balance, DateTime.Now, CurrentUser.UserID);

                        TransferLog.SaveTransferLog();
                        GetTransferLogs();
                    }
                }
                else
                    MessageBox.Show($"Transfer amount from the Client with account number '{DeliverClient.AccountNumber}'\nto the Client with account number '{ReceiverClient.AccountNumber}' has been canceled", "Operation Canceled", MessageBoxButtons.OK, MessageBoxIcon.Information);

            }
            else
                MessageBox.Show($"Please enter Transfer amount to be perform", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }

        private void TXTBTransferAmount_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (!char.IsControl(e.KeyChar))
            {
                // Allow digits and the decimal point (.)
        ...
        {
            DGVTransferLogs.DataSource = clsTransferLogs_BLL.GetTransferLogs();
        }

        void AddCurrentUserInfoToLoginRegister()
        {
            clsLoginRegister_BLL LoginRegister = new clsLoginRegister_BLL(CurrentUser.UserID, DateTime.Now);
            LoginRegister.SaveLoginRegister();
        }

        private void TCMainSections_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (TCMainSections.SelectedIndex == 3 && DGVLoginsRegister.Rows.Count == 0)
                GetLoginsRegister();
        }

        void GetLoginsRegister()
        {
            DGVLoginsRegister.DataSource = clsLoginRegister_BLL.GetLoginsRegister();
        }

        private void TCClients_Selecting(object sender, TabControlCancelEventArgs e)
        {
            int FeatureToBeAccess = Convert.ToInt32(TCClients.SelectedTab.Tag);
            if ((CurrentUser.Permissions & FeatureToBeAccess) != FeatureToBeAccess)
            {
                TCMainSections.SelectedIndex = 4;
                MessageBox.Show("You don't have permission to access this feature", "Access Denied", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
        }

        private void TCMainSections_Selecting(object sender, TabControlCancelEventArgs e)
        {
            int FeatureToBeAccess = Convert.ToInt32(TCMainSections.SelectedTab.Tag);

            if (CurrentUser.Permissions == -1 || FeatureToBeAccess == -1) return;
